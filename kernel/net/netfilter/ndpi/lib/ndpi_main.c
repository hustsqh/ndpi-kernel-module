...
#include <linux/netfilter/ndpi/ahocorasick.h>
#include <linux/netfilter/ndpi/libcache.h>

#define NDPI_CURRENT_PROTO NDPI_PROTOCOL_UNKNOWN

#include <linux/netfilter/ndpi/ndpi_api.h>
#include <linux/netfilter/ndpi/ndpi_config.h>
#ifndef __KERNEL__
#include <time.h>
#ifndef WIN32
#include <unistd.h>
...
#include <linux/netfilter/ndpi/ndpi_content_match.c.inc>
#include <linux/netfilter/ndpi/ndpi_patricia.h>
#include <linux/netfilter/ndpi/ht_hash.h>
...
static int init_hyperscan(struct ndpi_detection_module_struct *ndpi_mod) {
  ...
  ndpi_mod->hyperscan = (void*)ndpi_malloc(sizeof(struct hs));
...
#ifdef DEBUG
        printf("Fail to ndpi_calloc memory for %s\n", host_match[i].string_to_match);
#endif
      }
    }
...
void ndpi_exit_detection_module(struct ndpi_detection_module_struct *ndpi_struct) {
  if(ndpi_struct != NULL) {
    ...
    while(ndpi_struct->custom_categories.to_load != NULL) {
      ...
      ndpi_free(ndpi_struct->custom_categories.to_load->expression);
      ndpi_free(ndpi_struct->custom_categories.to_load);
...
#ifdef __KERNEL__
  ndpi_my_inet_pton(AF_INET, ip_address_and_mask, &pin);
#else
...
EXPORT_SYMBOL_GPL(NDPI_BITMASK_COMPARE);
...
EXPORT_SYMBOL_GPL(NDPI_BITMASK_IS_EMPTY);
