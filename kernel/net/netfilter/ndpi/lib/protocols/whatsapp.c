...
void ndpi_search_whatsapp(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  if(flow->l4.tcp.wa_matched_so_far == 0) {
    if(packet->payload_packet_len >= sizeof(whatsapp_sequence)){
        if(memcmp(packet->payload, whatsapp_sequence, sizeof(whatsapp_sequence)) == 0){
            ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_WHATSAPP, NDPI_PROTOCOL_UNKNOWN);
        }else{
            NDPI_EXCLUDE_PROTO(ndpi_struct, flow);
        }
    }else{
        if(memcmp(packet->payload, whatsapp_sequence, packet->payload_packet_len) == 0){
    ...
    if(packet->payload_packet_len + flow->l4.tcp.wa_matched_so_far >= sizeof(whatsapp_sequence)){
        if(memcmp(packet->payload, &whatsapp_sequence[flow->l4.tcp.wa_matched_so_far-1], 
            sizeof(whatsapp_sequence) - flow->l4.tcp.wa_matched_so_far) == 0){
            ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_WHATSAPP, NDPI_PROTOCOL_UNKNOWN);
        }else{
            NDPI_EXCLUDE_PROTO(ndpi_struct, flow);
        }
